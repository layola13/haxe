...
let make_binop ctx op e1 e2 is_assign_op p =
	...
	| OpBoolOr ->
		...
		mk_op e1 e2 b
	| OpInterval ->
		(* Use platform-appropriate package for IntIterator *)
		let pack = match ctx.com.platform with
			| Js | TypeScript -> []
			| _ -> ["std"]
		in
		let t = Typeload.load_instance ctx (make_ptp (mk_type_path (pack,"IntIterator")) null_pos) ParamNormal LoadNormal in
