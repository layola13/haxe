...
let get_config com =
	...
	| TypeScript ->
		let es6 = true in
		{
			...
			pf_scoping = {
				vs_scope = BlockScope;
				vs_flags = [NoShadowing; SwitchCasesNoBlocks;];
			};
			pf_supports_atomics = true;
		}
...
let init_platform com =
	...
	(* TypeScript should not forbid js package *)
	let forbid_list = if com.platform = TypeScript then
		"java" :: (List.filter (fun p -> p <> "js") (List.map platform_name platforms))
	else
		"java" :: (List.map platform_name platforms)
	in
	com.package_rules <- List.fold_left forbid com.package_rules forbid_list;
