...
let generate_class ctx c =
	...
	if has_class_flag c CInterface then begin
		...
		List.iter (fun f ->
			...
			else if has_class_field_flag f CfPrivate then spr ctx "private "
			else spr ctx "public ";
...
let alloc_ctx com =
	let ts_version = 
		try int_of_string (Gctx.defined_value com Define.TsVersion)
		with _ -> 5
	in
	...
	let ctx = {
		...
		output_mode = output_mode;
		emit_decorators = Gctx.defined com Define.TsDecorators;
		emit_namespaces = Gctx.defined com Define.TsNamespaces;
		strict_null_checks = Gctx.defined com Define.TsStrictNull;
		current = null_class;
		tabs = "";
		in_interface = false;
		in_value = None;
...
let generate com =
	...
	(* 生成.d.ts文件 *)
	if not (Gctx.defined com Define.TsNoDts) then begin
